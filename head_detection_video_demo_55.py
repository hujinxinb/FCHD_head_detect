...
def detect(img_read, write_path,head_detector,img_path,face_detect):
    ...
    for (i, rect) in enumerate(rects):
        cv2.rectangle(image_raw, (rect[0], rect[1]), (rect[0] + rect[2], rect[1] + rect[3]), (0,255,0),2)
    ...
    if SAVE_FLAG == 1:
       ...
       #cv2.imshow("frame_end",frame_end)
        ...
        if key == ord("q"):
            show=0
            print ("best frame is **********"+best_frame+"   num is *************"+str(people_num_detect))
    else:
        image_raw.show()    
...
def findPeopleNumHead(video_address, head_detector,face_detect):
    ...
    while ret and show:
        ...
        if (c % timeF == 0):
            ...
            people_num_cache = detect(img_read, write_path,head_detector,img_path,face_detect)
            if people_num_cache>people_num_detect:
                people_num_detect = people_num_cache
                best_frame = img_path
            print (video_address,c,people_num_cache)
        ret, frame = vs.read()
        c+=1
